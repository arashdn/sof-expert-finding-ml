...
def main(_):
    ...
    y_logit = tf.matmul(tf.matmul(v, wp), wc)
    ...
    cross_entropy = tf.add(
        tf.mul(param1, tf.reduce_sum(tf.mul(doc_len , tf.nn.softmax_cross_entropy_with_logits(logits=y_logit, labels=y_))))\
                    ,\
        tf.mul(param2, tf.nn.l2_loss(wc))\
        )
    ...
    while not eof:
        ...
        for ii in range(0, count_iter):
            ...
            sess.run(train_step, feed_dict={v: batch_xs, y_: batch_ys, doc_len: docp, batch_size: bs})

            avg_cost = sess.run(cross_entropy, feed_dict={v: batch_xs, y_: batch_ys, doc_len: docp, batch_size: bs})
        ...
        if cnt % 500 == 0:
            ...
            numpy.save('./save/wp'+str(cnt)+'.npy', wp.eval())
            numpy.save('./save/wc'+str(cnt)+'.npy', wc.eval())
            # numpy.save('./save/b'+str(cnt)+'.npy', b.eval())
            ff = open("./save/last_batch.txt", mode='w')
            print("Last batch = "+str(cnt), file=ff)
            ff.close()
            print("files saved")
